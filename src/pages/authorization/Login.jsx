			...
			if (email !== "" && !emailRegex.test(email)) {
				...
				try {
					let response = await fetch(
						"https://do-track-backend-production.up.railway.app/api/login",
						{
							method: "POST",
							headers: {
	...
	return (
		<div className="flex w-full h-screen items-center lg:items-end justify-center lg:justify-between bg-gradient-to-b from-primPurple to-primOrange md:bg-white">
			...
			<div className="lg:w-full hidden lg:flex justify-end items-end">
				<img
					src="/static/images/login_illustrator.png"
					alt=""
					className="w-[700px]"
				/>
			</div>
				...
				<form>
					<div className="flex flex-col mx-0 lg:mx-20 pt-4 gap-5">
						<div className="flex flex-col gap-1">
							<label
								className={`${
									emailError ? "text-red-500 font-medium" : "text-gray-500 font-medium"
								}`}>
								Email
							</label>
							<input
						...
						<div className="flex flex-col gap-1 relative">
							<label
								className={`${
									passError ? "text-red-500 font-medium" : "text-gray-500 font-medium"
								}`}>
								Password
							</label>
							<input
